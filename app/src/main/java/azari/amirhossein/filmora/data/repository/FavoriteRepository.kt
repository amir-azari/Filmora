...
class FavoriteRepository @Inject constructor(
    ...
    fun getFavoriteMoviesPager(): Flow<PagingData<ResponseMoviesList.Result>> = flow {
        ...
        emitAll(
            Pager(
                ...
                pagingSourceFactory = {
                    FavoriteMoviesPagingSource(sessionManager, remote, accountId)
                }
            ).flow
        )
    }.flowOn(Dispatchers.IO)

    fun getFavoriteTVsPager(): Flow<PagingData<ResponseTvsList.Result>> = flow {
        val sessionId = sessionManager.getSessionId().firstOrNull()
        val accountId = accountDataStore.getCachedAccountDetails().firstOrNull()?.id
        if (sessionId.isNullOrEmpty() || accountId == null) {
            throw Exception("Session or Account ID not found")
        ...
        emitAll(
            Pager(
                config = PagingConfig(pageSize = 20, enablePlaceholders = false),
                pagingSourceFactory = {
                    FavoriteTVPagingSource(sessionManager, remote, accountId)
                }
            ).flow
        )
    }.flowOn(Dispatchers.IO)
